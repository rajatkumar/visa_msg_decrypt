
VisaPayloadDecrypt = require '../src/index'

chai = require 'chai'
chai.should()
expect = chai.expect

testPayload = {
  "encKey": "ck/TCCMSU6/lZPck0mXbpXmdaY/cNqaXz+xqx92Debr+5ALG5ZKVndl4He4JOjfNELE0yBBvEF+gD1FpTDRXdl6j/BkmcNqRsPiU59p7+9ccLlNfdsuf6rhaeuERnSns",
  "encPaymentData": "",
}
testPayload_no_key = {
  "encPaymentData": "",
}
testPayload_no_data = {
  "encKey": "ck/TCCMSU6/lZPck0mXbpXmdaY/cNqaXz+xqx92Debr+5ALG5ZKVndl4He4JOjfNELE0yBBvEF+gD1FpTDRXdl6j/BkmcNqRsPiU59p7+9ccLlNfdsuf6rhaeuERnSns",
}

testSecret = "5OEz37YWRTwYQJ8}3G#NQVpC5ct2BD5iztS1rxn2"
invalidSecret = "iaminvalid"

describe 'VisaPayloadDecrypt ', ->
  Decrypt = null
  it 'should decrypt successfully', ->
    Decrypt = new VisaPayloadDecrypt testSecret, true
    decryptedJson = Decrypt.decrypt testPayload
    #console.log decryptedJson
    decryptedJson.userData.userEmail.should.equal('rajat.kumar@payrange.com')
    expect(decryptedJson.paymentInstrument).to.exist
    expect(decryptedJson.paymentInstrument.paymentType).to.exist
  it 'should throw exception for bad secret', ->
    Decrypt = new VisaPayloadDecrypt invalidSecret, false
    (-> Decrypt.decrypt testPayload).should.throw 'Invalid payload or key. HMAC verification failed.'
  it 'should throw exception for payload without key', ->
    Decrypt = new VisaPayloadDecrypt testSecret, false
    (-> Decrypt.decrypt testPayload_no_key).should.throw 'Payload does not contain encKey.'
  it 'should throw exception for payload without Payload Data', ->
    Decrypt = new VisaPayloadDecrypt testSecret, false
    (-> Decrypt.decrypt testPayload_no_data).should.throw 'Payload does not contain encPaymentData.'
  it 'should throw exception for bad Object constructor', ->
    Decrypt = new VisaPayloadDecrypt()
    (-> Decrypt.decrypt testPayload).should.throw 'Invalid secret key.'







